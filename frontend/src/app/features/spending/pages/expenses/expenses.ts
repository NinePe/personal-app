...
export class ExpensesDashboard implements OnInit {
  ...
  spendingSuggestions = signal<SpendingSuggestion[]>([]);
  projection = signal<{ monthly: MonthlyProjection[] }>({ monthly: [] });
  ...
  private loadSpendingSuggestions(refresh = false) {
    this.suggestionsLoading.set(true);
    this.suggestionsSvc.getSpendingSuggestions(refresh).subscribe(res => {
      this.spendingSuggestions.set(res.suggestions);
      this.projection.set(res.projection);
      this.suggestionsLoading.set(false);
    });
  }

  refreshSuggestions() {
    this.loadSpendingSuggestions(true);
  }
