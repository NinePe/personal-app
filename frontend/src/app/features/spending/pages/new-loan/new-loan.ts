...
import {
  LoansService, NewLoan, LoanPersonSummary, LoansSummary
} from '@app/services/loans.service';
...
export class NewLoanPage implements OnInit {
  ...
  private loansSvc   = inject(LoansService);
  ...
  ngOnInit() {
    ...
    forkJoin({
      ...
      summary: this.loansSvc.getLoansSummary(),
      ...
      if (id) {
        this.editId.set(id);
        this.loansSvc.getLoanById(id).subscribe(loan => {
  ...
  save() {
    ...
    const obs = this.isEdit()
      ? this.loansSvc.updateLoan(this.editId()!, body)
      : this.loansSvc.createLoan(body);
