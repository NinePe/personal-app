...
import { CardsService, Card, BillingCycle, CARD_COLORS } from '@app/services/cards.service';
import { ExpensesService, MonthStat, PersonSplit, SplitDetailRow } from '@app/services/expenses.service';
...
export class Payments implements OnInit {
  ...
  loadForCard(cardId: string) {
    ...
    if (card?.type === 'debit') {
      const key = `${y}-${m}`;
      forkJoin({
        stats:  this.expensesSvc.getMonthlyStats(cardId, y),
        splits: this.expensesSvc.getPeopleSplit(m, y, cardId),
      ...
      forkJoin({
        cycles: this.cardsSvc.getCycles(cardId),
        splits: this.expensesSvc.getPeopleSplit(m, y, cardId),
  ...
  selectMonth(stat: MonthStat) {
    ...
    this.expensesSvc.getPeopleSplitByRange(stat.from_date, stat.to_date, cardId)
      .subscribe(splits => {
        this.splitPeople.set(splits);
  ...
  selectCycle(cycle: BillingCycle) {
    ...
    if (!cardId) return;
    this.splitLoading.set(true);
    this.expensesSvc.getPeopleSplitByRange(cycle.from_date, cycle.to_date, cardId)
      .subscribe(splits => {
        this.splitPeople.set(splits);
        this.splitLoading.set(false);
      });
  ...
  saveNew() {
    ...
    this.cardsSvc.createCycle(cardId, f).subscribe({
      next: () => {
        this.cardsSvc.getCycles(cardId).subscribe(cycles => {
          this.cycles.set(cycles);
          this.showNewForm.set(false);
  ...
  saveEdit(cycle: BillingCycle) {
    ...
    this.saving.set(true);
    this.cardsSvc.updateCycle(cycle.id, f).subscribe({
      next: updated => {
  ...
  deleteCycle(id: string) {
    ...
    this.cardsSvc.deleteCycle(id).subscribe(() => {
  ...
  toggleCyclePaid(cycle: BillingCycle, ev: Event) {
    ...
    this.cardsSvc.setCyclePaid(cycle.id, newPaid).subscribe({
      next: () => {
  ...
  openSplitDetail(person?: PersonSplit) {
    ...
    this.expensesSvc.getSplitDetail(from, to, cardId, personId).subscribe(rows => {
