...
import { ExpensesService, PersonSplit } from '@app/services/expenses.service';
...
export class People implements OnInit {
  ...
  ngOnInit() {
    this.peopleSvc.getPeople().subscribe(people => {
  ...
  loadSplits() {
    ...
    if (f === 'month') {
      obs = this.expensesSvc.getPeopleSplit(this.currentMonth, this.currentYear);
    } else if (f === 'year') {
      obs = this.expensesSvc.getPeopleSplitByRange(`${this.currentYear}-01-01`, `${this.currentYear}-12-31`);
    } else if (f === 'all') {
      obs = this.expensesSvc.getPeopleSplitByRange('2000-01-01', '2099-12-31');
    } else {
      const from = this.customFrom(), to = this.customTo();
      if (!from || !to) { this.loading.set(false); return; }
      obs = this.expensesSvc.getPeopleSplitByRange(from, to);
    }
  ...
  deletePerson(id: string) {
    ...
    this.peopleSvc.deletePerson(id).subscribe({
      next: () => {
