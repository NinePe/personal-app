...
import { CardsService, Card, BillingCycle } from '@app/services/cards.service';
import { ExpensesService, MonthStat } from '@app/services/expenses.service';
...
export class CyclesHistory implements OnInit {
  ...
  ngOnInit() {
    ...
    this.cardsSvc.getCards().subscribe(cards => {
      ...
      if (c.type === 'credit') {
        ...
        forkJoin([
          this.expensesSvc.getMonthlyStats(cardId, currentYear),
          this.expensesSvc.getMonthlyStats(cardId, currentYear - 1),
          this.expensesSvc.getMonthlyStats(cardId, currentYear - 2),
        ]).subscribe(([a, b, cc]) => {
          const all = [...a, ...b, ...cc]
            .filter(s => +s.total > 0)
  ...
  togglePaid(row: Row, ev: Event) {
    ...
    this.cardsSvc.setCyclePaid(row.id, newPaid).subscribe({
      error: () => {
        this.allRows.update(list => list.map(r =>
